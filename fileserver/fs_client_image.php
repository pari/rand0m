<?php
// image.php/{$fileToken_md5}?height=800&filetoken={$fileToken}&appname=plutohr_storecash
...
$fs_url = "{$FILESERVER_CONFIGURATION['BASEURL']}getfile.php?appName={$_GET['appname']}&appKey={$FILESERVER_CONFIGURATION['MYAPPS'][$_GET['appname']]}&fileToken={$_GET['filetoken']}";
$file_info = file_get_contents($fs_url);
$file_info = json_decode( $file_info , true );
// $file_info['filedata']['originalFileName']
// $file_info['filedata']['fileSize']
$image = CACHE_DIR.$file_info['filedata']['file_diskName'] ;
$fp = fopen( $image , 'w');
fwrite($fp, base64_decode($file_info['filedata']['file_b64_content']));
fclose($fp);
