...
$db_conn = mssql_connect("192.168.1.1","sa","cccc-xxx") or die( "ERROR: Connection to MYSERVER failed" );
mssql_select_db( "somedatabase", $db_conn ) or die( "ERROR: Selecting database failed" );

function CT_CARDNUMBER($cardno){
	$first = substr( $cardno , 0, -6);
	$last6 = substr( $cardno , -6);
	return $first."CT".$last6 ;
}
...
class sendaMail{
    ...
    public function send() {
		...
		$mail->Password   = "vvvvvv";            // GMAIL password
