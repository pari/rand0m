...
setTimeout(function(){
	...
	// finished adding icons on the page
	// now start adding events
	icon_archive.addEventListener('click', function(e){
		var win_listMonths = Titanium.UI.createWindow({ url: 'mydiary.js', title: 'My Diary' , backgroundColor:'#FFF', barColor:'#AD7A12', backButtonTitle :'Home' });
		Titanium.UI.currentTab.open(win_listMonths,{animated:true});
	});
	
	
	pic_archive.addEventListener('click', function(e){
		var win_listPics = Titanium.UI.createWindow({ url: 'pics.js', title: 'Pictures' , backgroundColor:'#C8CDFF', barColor:'#AD7A12', backButtonTitle :'Home' });
		Titanium.UI.currentTab.open(win_listPics, {animated:true});
	});
