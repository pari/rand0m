...
var My_JsLibrary = {
	...
	parseGETparam : function(getparam ){ // My_JsLibrary.parseGETparam( 'getvarname');
	...
	updatePageWithGetVar: function(varname, newval){ // My_JsLibrary.updatePageWithGetVar('varname', 'newvalue');
		...
		var curl = window.location.href;
		if( curl.contains('?') ){
			var currentVarNameValue = My_JsLibrary.parseGETparam( varname);
			if( currentVarNameValue ){
				var tmp_1 = varname + '='+ currentVarNameValue ;
				window.location.href = curl.replaceXY(tmp_1 , varname + '='+ newval );
				return;
			}else{
				window.location.href = curl + '&' + varname + '='+ newval ;
				return;
			}
		}else{
			window.location.href = curl + '?' + varname + '='+ newval ;
	...
	amountInWords: function ( amount ) { // My_JsLibrary.amountInWords( amount )
		var amount = String(Math.floor(Number(amount)));
		var ones_sp = { '1': 'One', '2': 'Two' , '3' : 'Three', '4' : 'Four' ,'5' :'Five' ,'6' : 'Six' ,'7' :'Seven','8':'Eight', '9': 'Nine', '0' :'' };
		var tens_sp = { '10':'Ten' , '11': 'Eleven', '12': 'Twelve', '13': 'Thirteen', '14': 'Fourteen' , '15': 'Fifteen', '16' : 'Sixteen' , '17': 'Seventeen' , '18': 'Eighteen' , '19': 'Nineteen' };
		var tys_sp = { '2': 'Twenty' , '3' : 'Thirty', '4' : 'Forty' ,'5' :'Fifty' ,'6' : 'Sixty' ,'7' :'Seventy','8':'Eighty', '9': 'Ninety' };
		var noOfDigits = amount.length;
		switch (noOfDigits) {
		    case 1: // if 1 - 'x' rs
				return ones_sp[amount] ;
			break;
			case 2: // if 2 - 'xx' Rs
				var fchar = amount.charAt(0);
				if( fchar == '1'){ return  tens_sp[amount] ; }
				return tys_sp[fchar] + ' '+ My_JsLibrary.amountInWords(amount.charAt(1)) ;
	        break;
			case 3: // if 3 - 'X' hundred + amountInWords(last 2)
				var hchar = amount.charAt(0);
				var tens = amount.slice(-2);
				if( Number(tens) ){
					return ones_sp[hchar] + ' Hundred and ' + My_JsLibrary.amountInWords(tens);
				}else{
					return ones_sp[hchar] + ' Hundred' ;
				}

			break;
			case 4: // if 4 - 'X' thousand
				var tchar = amount.charAt(0);
				return ones_sp[tchar] + ' Thousand, ' + My_JsLibrary.amountInWords(amount.slice(-3));
			break;
			case 5: // if 5 - 'XX' thousand
				var first_two = amount.substr(0,2);
				return My_JsLibrary.amountInWords(first_two) + ' Thousands, ' + My_JsLibrary.amountInWords(amount.slice(-3));
			break;
			case 6: // if 6 - 'x' Lakh
				var tchar = amount.charAt(0);
				return ones_sp[tchar] + ' Lakh, ' + My_JsLibrary.amountInWords(amount.slice(-5));
			break;
			case 7: // if 7 - 'xx' Lakhs
				var first_two = amount.substr(0,2);
				return My_JsLibrary.amountInWords(first_two) + ' Lakhs, ' + My_JsLibrary.amountInWords(amount.slice(-5));
			break;
			case 8: // if 8 - 'x' Crore
				var tchar = amount.charAt(0);
				return ones_sp[tchar] + ' Crore, ' + My_JsLibrary.amountInWords(amount.slice(-7));
			break;
			case 9: // if 9 - 'xx' Crores
				var first_two = amount.substr(0,2);
				return My_JsLibrary.amountInWords(first_two) + ' Crores, ' + My_JsLibrary.amountInWords(amount.slice(-7));
			break;
			case 10: // if 10 - 'xxx' Crores
				var first_3 = amount.substr(0,3);
				return My_JsLibrary.amountInWords(first_3) + ' Crores, ' + My_JsLibrary.amountInWords(amount.slice(-7));
			break;
		}
		return '' ;
	},
